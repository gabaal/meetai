...
export async function POST(req: NextRequest) {
    ...
    if (eventType === "call.session_started") {
        ...
        if (!meetingId) {
            return NextResponse.json({ error: 'Missing meetingId in call session participant left event' }, { status: 400 })
        }
        const call = streamVideo.video.call("default", meetingId)
        await call.end()
        ...
        const meetingId = event.call.custom?.meetingId
        if (!meetingId) {
            return NextResponse.json({ error: 'Missing meetingId in call session ended event' }, { status: 400 })
        }
        await db
            .update(meetings)
