...
export const Transcript = ({ meetingId }: Props) => {
    ...
    return (
        <div className="bg-white rounded-lg border px-4 py-5 flex flex-col gap-y-4 w-full">
            ...
            <ScrollArea>
                <div className="glex flex-col gap-y-4">
                            ...
                            <div key={item.start_ts} className="flex flex-col gap-y-2 hover:bg-muted p-4 rounded-md border">
                                ...
                                <div className="flex gap-x-2 items-center">
                                    <Avatar className="size-6">
                                        <AvatarImage
                                        src={(item.user.image ?? generateAvatarUri({seed:item.user.name, variant:'initials'} ))} alt={item.user.name} />
                                    </Avatar>
                                    <p className="text-sm font-medium">{item.user.name}</p>
                                    <p className="text-sm text-blue-500 font-medium">{format(
